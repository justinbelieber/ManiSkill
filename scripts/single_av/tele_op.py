...
def solve(env: BaseEnv, debug=False, vis=False):
    ...
    # 遍历所有插件，找到 TransformWindow，让transform_window 指向它
    ...
    while True:
        ...
        # rotation commands
            ...
            select_panda_hand()
            R = sapien.Pose(q=q_from_euler(0, 0, np.deg2rad(5)))
            transform_window.gizmo_matrix = (transform_window._gizmo_pose * R).to_transformation_matrix()
            transform_window.update_ghost_objects()
        elif viewer.window.key_press("8"):
            select_panda_hand()
            R = sapien.Pose(q=q_from_euler(0, 0, -np.deg2rad(5)))
            transform_window.gizmo_matrix = (transform_window._gizmo_pose * R).to_transformation_matrix()
            transform_window.update_ghost_objects()
        ...
        # 额外：绕 Y 轴顺时针 (y)，逆时针 (h)
            ...
            select_panda_hand()
            R = sapien.Pose(q=q_from_euler(0, np.deg2rad(5), 0))
            transform_window.gizmo_matrix = (transform_window._gizmo_pose * R).to_transformation_matrix()
            transform_window.update_ghost_objects()
        ...
        # 额外：绕 X 轴顺时针 (x)，逆时针 (z)
            ...
            select_panda_hand()
            R = sapien.Pose(q=q_from_euler(np.deg2rad(5), 0, 0))
            transform_window.gizmo_matrix = (transform_window._gizmo_pose * R).to_transformation_matrix()
            transform_window.update_ghost_objects()
